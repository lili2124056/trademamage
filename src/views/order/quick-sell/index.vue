  ...
  <div class="app-container">
    <el-card class="box-card">
      ...
      >
        ...
        <el-table-column label="收款方式" width="100px" align="center">
          <template slot-scope="{row}">
            <span>{{ row.receiveMethod }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogVisible" width="500px">
      <el-form ref="dataForm" :rules="rules" :model="currentOrder" label-position="left" label-width="100px">
        ...
        <el-form-item label="收款方式" prop="receiveMethod">
          <el-select v-model="currentOrder.receiveMethod" placeholder="请选择收款方式" style="width: 100%">
            <el-option label="银行卡" value="银行卡" />
            <el-option label="支付宝" value="支付宝" />
            <el-option label="微信" value="微信" />
          </el-select>
        </el-form-item>
        <el-form-item label="收款账号" prop="receiveAccount" v-if="currentOrder.receiveMethod">
          <el-input v-model="currentOrder.receiveAccount" placeholder="请输入收款账号" />
        </el-form-item>
        <el-form-item label="收款人姓名" prop="receiveName" v-if="currentOrder.receiveMethod">
          <el-input v-model="currentOrder.receiveName" placeholder="请输入收款人姓名" />
        </el-form-item>
    ...
    <!-- 详情对话框 -->
    <el-dialog title="卖单详情" :visible.sync="detailDialogVisible" width="600px">
      <el-descriptions :column="1" border>
        ...
        <el-descriptions-item label="收款方式">{{ currentOrder.receiveMethod }}</el-descriptions-item>
        <el-descriptions-item label="收款账号" v-if="currentOrder.receiveAccount">{{ currentOrder.receiveAccount }}</el-descriptions-item>
        <el-descriptions-item label="收款人姓名" v-if="currentOrder.receiveName">{{ currentOrder.receiveName }}</el-descriptions-item>
...
export default {
  ...
  data() {
    return {
      tableKey: 0,
      list: [
        { orderNo: 'QS202506240001', currency: 'BTC', amount: '0.5', price: '30000', total: '15000', receiveMethod: '银行卡', receiveAccount: '6222021234567890123', receiveName: '张三', status: '待处理', createdTime: Date.now() - 3600 * 1000 },
        { orderNo: 'QS202506240002', currency: 'ETH', amount: '10', price: '2000', total: '20000', receiveMethod: '支付宝', receiveAccount: 'zhangsan@example.com', receiveName: '张三', status: '已接单', createdTime: Date.now() - 7200 * 1000, acceptTime: Date.now() - 3600 * 1000 },
        { orderNo: 'QS202506240003', currency: 'USDT', amount: '5000', price: '1', total: '5000', receiveMethod: '微信', receiveAccount: 'wxid_123456', receiveName: '张三', status: '已完成', createdTime: Date.now() - 86400 * 1000, acceptTime: Date.now() - 82800 * 1000, completedTime: Date.now() - 79200 * 1000 },
        { orderNo: 'QS202506230001', currency: 'BTC', amount: '0.2', price: '29000', total: '5800', receiveMethod: '银行卡', receiveAccount: '6222021234567890123', receiveName: '李四', status: '已取消', createdTime: Date.now() - 172800 * 1000, remark: '客户取消订单' }
      ],
      total: 4,
      listLoading: false,
      listQuery: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          // 实际项目中，这里应该调用API创建订单
          this.currentOrder.orderNo = 'QS' + parseTime(new Date(), '{y}{m}{d}') + Math.floor(Math.random() * 10000).toString().padStart(4, '0')
